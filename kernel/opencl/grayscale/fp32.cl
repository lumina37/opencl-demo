__kernel void clcmain(__read_only image2d_t src, __write_only image2d_t dst) {
    ...
    float4 pixel = read_imagef(src, sampler, tid);

    const float4 weight = (float4)(0.299f, 0.587f, 0.114f, 0.0f);
    float gray = dot(weight, pixel);

    float4 outputPixel = (float4)(gray, gray, gray, pixel.w);
    write_imagef(dst, tid, outputPixel);
}
